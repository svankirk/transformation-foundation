	...
	_handler.handle = function() {
		try {
			...
			if (parsedReq.pathname !== settings.getUrl) {
				...
				{
					throw "Unknown function request";
				}
...
//				sendError(settings.FAILURE, "" + ex);
				...
				thisdb.addDoc(query2, onCompletion);
			}
	...
	try {
		if ( settings.usehttps ) {
			https.createServer(options, function(req, resp) {
				...
				console.log("Started listening for HTTPS connection at: " + settings.listenPort);
			});
		}
		else {
			http.createServer(function(req, resp) {
				...
				console.log("Started listening for HTTP connection at: " + settings.listenPort);
			});
